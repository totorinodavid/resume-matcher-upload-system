...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Seed credits for a NextAuth user")
    p.add_argument("--user", "-u", dest="user", default="test-user", help="User id to seed (default: test-user)")
...
async def run(user: str, delta: int, reason: str, event: Optional[str]) -> int:
    async with AsyncSessionLocal() as s:
        ...
        await svc.credit_purchase(user_id=user, delta=delta, reason=reason, stripe_event_id=event)
        await s.commit()
        bal = await svc.get_balance(user_id=user)
        return int(bal)
