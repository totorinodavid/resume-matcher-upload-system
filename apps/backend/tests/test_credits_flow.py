...
async def test_credits_service_balance_and_debit(db_session, test_user_id):
    ...
    await svc.ensure_customer(user_id=test_user_id)
    assert await svc.get_balance(user_id=test_user_id) == 0

    # Credit +100
    await svc.credit_purchase(user_id=test_user_id, delta=100, reason="test", stripe_event_id="evt_1")
    await db_session.commit()
    assert await svc.get_balance(user_id=test_user_id) == 100
    ...
    # Debit 30
    await svc.debit_usage(user_id=test_user_id, delta=30, reason="use")
    await db_session.commit()
    assert await svc.get_balance(user_id=test_user_id) == 70

    # Over-debit should raise
    with pytest.raises(InsufficientCreditsError):
        await svc.debit_usage(user_id=test_user_id, delta=100, reason="use")
...
async def test_credits_endpoints(db_session, monkeypatch, test_user_id):
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://testserver") as client:
        ...
        svc = CreditsService(db_session)
        await svc.credit_purchase(user_id=test_user_id, delta=50, reason="test", stripe_event_id="evt_api")
        await db_session.commit()
