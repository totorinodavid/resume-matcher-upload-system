...
interface PageParams { params?: { locale?: string; resume_id?: string } }
...
export default async function ResumeDetailPage({ params }: PageParams) {
  const session = await auth();
  if (!session) {
    redirect("/login");
  }
  const locale = params?.params?.locale || 'en';
  const resume_id = params?.params?.resume_id || '';
